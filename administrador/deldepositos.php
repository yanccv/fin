		...
		<div class="FormDatos">
       	  <form method="post" action="procesar.php" name="fdatos" id="fdatos">
          	<div class="FormTitulo">
            	Formulario de Eliminacion de Depositos
            </div>
            <div class="CampoCompleto">
                <?php
                    $ConDep=$bd->dbConsultar("select b.banco,d.referencia,d.fecha,d.monto_oficial,d.monto_base from movimientos as d inner join cuentas as c on d.cuenta=c.id inner join bancos as b on b.id=c.banco where d.estado='N'");
                    if ($bd->Error) {
                        echo "<center>".$bd->MsgError."</center>";
                    } else {
            ...
            <div class="FormFin">
                <div class="CampoLargo">
                    <input name="Boton2" id="Boton2" value="Eliminar" type="submit" />
                </div>
            </div>
      	  </form>
        </div>
