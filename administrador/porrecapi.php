<?php
	...
	$bd->dbConectar();
    if ($_GET['tipoform']=="E"){
        $CCliente=$bd->dbConsultar("select c.cedula,c.nombre,c.apellido,c.fregistro,c.fupdate,c.minimoap,c.pais idpais,p.pais,p.monedaoficial idmoneda,m.moneda,m.cambio,ms.moneda basemoneda,f.monto,f.inicio,f.fin from clientes as c inner join franquiciados as f on f.cliente=c.cedula inner join paises as p on p.id=c.pais inner join monedas as m on m.id=p.monedaoficial inner join monedas as ms on ms.id=m.monedabase where cedula=?",array($_GET['cedula']));
        if ($bd->Error){
      ...
      <div id="FilaMenu">
		...
		<div class="FormDatos">
            ...
            <div class="CampoCompleto">
                ...
                <input name="idform" type="hidden" id="idform" value="recapi" />
            ...
            <div class="CampoCompleto">
                <?php
                    $ConDep=$bd->dbConsultar("select b.banco,d.referencia,d.fecha,d.monto_oficial,d.monto_base from movimientos as d inner join cuentas as c on d.cuenta=c.id inner join bancos as b on b.id=c.banco where d.cliente=? and d.estado='R' and franquicia='REC'",array($Cliente['cedula']));
                    echo $bd->getSql();
                    if ($bd->Error){
                        echo "<center>".$bd->MsgError."</center>";
                    }else{
