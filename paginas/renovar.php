<?php
   ...
   if (!$bd->Error){
    ...
    $ConConfig=$bd->dbConsultar("select c.tiempoactivo tiempo,c.minimoinicial minimo,c.minimorenova rminimo,c.mmaximo maximo,m.moneda ,c.conveniofpc as convenio from configuracion as c inner join monedas as m on m.id=c.monedabase where c.id=1");
    if (!$bd->Error){
        $FConfig=$ConConfig->fetch_array();

    }

   $RDiferido=RDiferido($bd,$_SESSION['cliente']['cedula']);
   $MaxRenova=round(($FConfig['maximo']-$RDiferido['mbase']),2);
   $MinRenova=$FConfig['rminimo']-$RDiferido['mbase'];
   if ($MinRenova<0)
    $MinRenova=0;
/*
    echo "<pre>";
...
<div id="Contenedor">
    ...
    <!-- InstanceBeginEditable name="CentroClientes" -->
        ...
        <div class="Articulo" id="FormDatos">
                <div class="TituloArticulo">Nueva Renovacion</div>
                <div class='SeparadorArticuloInterno'></div>
                <?php
                    if ($RDiferido['mbase']==$FConfig['maximo'])    echo "<center>Disculpe Ya Posees Depositos Por El Maximo de La Renovacion</center>";
                    else {
                ?>
                ...
                <div class="ContenidoArticulo">
                    <div class="CampoCompleto">
                        <div class="EtiquetaLarga">Monto Actual de Participacion: </div>
                        <div class="CampoCorto"><?php echo $Datos['monto']. " ".$Datos['MonedaBase']; ?> </div>
                        <div class="EtiquetaLarga">Monto Maximo de Participacion: </div>
                        <div class="CampoCorto"><?php echo $FConfig['maximo']. " ".$Datos['MonedaBase']; ?> </div>
                        <div class="Limpiador"></div>
                    </div>
                    <div class="CampoCompleto">
                        <div class="EtiquetaLarga">Monto Minimo de Renovacion: </div>
                        <div class="CampoCorto"><?php echo $FConfig['rminimo']. " ".$Datos['MonedaBase']; ?> </div>
                        <div class="EtiquetaLarga">Monto Diferido de Renovacion: </div>
                        <div class="CampoCorto"><?php echo ((double) $RDiferido['mbase']). " ".$Datos['MonedaBase']; ?> </div>
                        <div class="Limpiador"></div>
                    </div>
                    <div class="CampoCompleto">
                        <div class="EtiquetaLarga" >Tasa de Cambio : </div>
                  ...
                  <fieldset>Datos del Deposito
                    <div class="CampoCompleto">
                        ...
                        <?php
                           echo $bd->dbComboSimple("select b.id,b.banco from bancos as b inner join cuentas as c on c.banco=b.id where b.estado='A' and b.pais=? and c.estado='A' and c.cliente is null group by b.id",array($_SESSION['cliente']['idpais']),"CBanco",0,array(1),null);
                        ?>
                        </div>
                        ...
                        <div class="CampoCorto">
                           ...
                           <input type="hidden" id="idform" name="idform" value="CRenovar" />
                           <input type="hidden" id="capital" name="capital" value="<?php echo $MaxRenova; ?>" />
                           <input type="hidden" id="maximo" name="maximo" value="<?php echo $FConfig['maximo']; ?>" />
                           <input type="hidden" id="minimo" name="minimo" value="<?php echo $MinRenova; ?>" />
        ...
        <div class="Articulo" id="FormClave">
                <div class="TituloArticulo">Listado de Renovaciones</div>
                <div class='SeparadorArticuloInterno'></div>
                <br />
                <table id="TRetiros" class="Tabla" align='center' >
                    <thead>
                     ...
                     <?php
                        $ConMovimiento=$bd->dbConsultar("select m.id,b.banco,c.cuenta,m.referencia,m.movimiento,m.fautoriza,monto_base,m.estado from movimientos as m inner join cuentas as c on c.id=m.cuenta inner join bancos as b on b.id=c.banco where m.cliente=? and m.franquicia='REN' and m.estado<>'I' ",array($_SESSION['cliente']['cedula']));
                        if ($bd->Error){
                           echo $bd->MsgError;
                        }else{
                        ...
                        <tr id="<?php echo $Movimiento['id']; ?>">
                            ...
                            <?php
                                switch($Movimiento['estado']){
                                    ...
                                    case "V":   $estado="En Proceso";   break;
                                }
                            ?>
                            <td align='center' ><?php echo $estado; ?></td>
                            <td width="25"><?php if ($Movimiento['estado']=="V"){?> <a id="BorrarRen" rel="<?php echo $Movimiento['id']; ?>" href="#" alt="Eliminar Retiro"><img src="../imagenes/eliminar.png" border='0'/></a><?php } ?></td>
                        </tr>
                    <?php
                        }
                    ?>
