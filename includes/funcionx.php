<?php
   ...
   session_start(); 
    ...
    function FData($fecha){   return substr($fecha,6,4).substr($fecha,2,4).substr($fecha,0,2);  }
    function FUser($fecha){   return substr($fecha,8,2)."/".substr($fecha,5,2)."/".substr($fecha,0,4);  }
    function FTData($fecha){   return substr($fecha,6,4).substr($fecha,2,4).substr($fecha,0,2).substr($fecha,10);  }
    function FTUser($fecha){   return substr($fecha,8,2).substr($fecha,4,4).substr($fecha,0,4).substr($fecha,10);  }
...
*/
   ...
   function RecorrerAsociadosPublicidad($bd,$asociador,&$nivelp,&$porcp,&$montop){
      //$ConAsociados=$bd->dbConsultar("select cedula,estado from clientes where asociador=?",array($asociador));      
      $ConAsociados=$bd->dbConsultar("select c.cedula,c.estado,c.fpm from clientes as c left join franquiciados as f on f.cliente=c.cedula where c.asociador=?",array($asociador));      
      if ($bd->Error){
         echo $bd->MsgError;
         exit();
      }else{
         ...
         if ($ConAsociados->num_rows>0){            
            while ($FAsociado=$ConAsociados->fetch_array()){
               ...
               if ($FAsociado['estado']=='A' && $FAsociado['fpm']==1){                                    
                  $ConMonto=$bd->dbConsultar("select sum(monto_base) as monto from movimientos where movimiento='Deposito' and franquicia in ('BAN','CLA') and estado='A' and month(fautoriza)=? and year(fautoriza)=? and cliente=?",array(date("m"),date("Y"),$FAsociado['cedula']));
                  //echo $bd->getSql()."<br />";
                  if ($bd->Error){
                     echo $bd->MsgError;
                     exit();
                     ...
                     //echo "Asociado: ".$FAsociado['cedula'].", Pertenece a FPM ".$FMonto['monto']."*".(($porcp/2)/100)."=".$montop."<br />";
                     //$porcp=round(($porcp/2),2);
                     RecorrerAsociadosPublicidad($bd,$FAsociado['cedula'],$nivelp,round($porcp/2,2),$montop);                     
                     
                  }
               }else{
                  //echo "Asociado: ".$FAsociado['cedula'].", No Pertenece a FPM , Porcentaje: ".$porcp.",  Monto: ".$montop."<br />";
                  RecorrerAsociadosPublicidad($bd,$FAsociado['cedula'],$nivelp,$porcp,$montop);
               }
                         
                        
               //echo "<br />".$FAsociado['cedula']." Nivel: ".$nivel." NewPorc: ".$newporc." Monto: ".$montop."<br />";
               //RecorrerAsociadosPublicidad($bd,$FAsociado['cedula'],$nivelp,$porcp,$montop);
            }
         }
      }
   ...
   function RecorrerArbol($bd,$cedula=null){
      ...
      if (is_null($cedula)){
         $ConAsociado=$bd->dbConsultar("select c.cedula,f.monto,m.cambio,c.fpm from clientes as c inner join franquiciados as f on f.cliente=c.cedula inner join paises as p on p.id=c.pais inner join monedas as m on m.id=p.monedaoficial where c.estado='A' and c.asociador is null");
      }else{
         $ConAsociado=$bd->dbConsultar("select c.cedula,f.monto,m.cambio,c.fpm from clientes as c inner join franquiciados as f on f.cliente=c.cedula inner join paises as p on p.id=c.pais inner join monedas as m on m.id=p.monedaoficial where c.estado='A' and c.asociador=?",array($cedula));   
      }
      ...
      if (!$bd->Error){
         if ($ConAsociado->num_rows>0){            
            while ($Asociado=$ConAsociado->fetch_array()){
               ...
               $bd->dbInsertar("insert into movimientos (id,cliente,referencia,franquicia,movimiento,fecha,fautoriza,monto_oficial,monto_base,estado) values(lastid('movimientos'),?,lastmv('Liquidez','".$Asociado['cedula']."'),'FCG','Liquidez',curdate(),curdate(),?,?,'A')",array($Asociado['cedula'],$MontoOficial,$MontoBase));
               if ($bd->Error){
                  $bd->RollBack();
                  echo $bd->MsgError;
               }    
/*                                                                                                           
               if ($Asociado['fpm']==1){  
                  $bd->dbInsertar("insert into movimientos (id,cliente,referencia,franquicia,movimiento,fecha,fautoriza,monto_oficial,monto_base,estado) values(lastid('movimientos'),?,lastmv('Liquidez','".$Asociado['cedula']."'),'FPM','Liquidez',curdate(),curdate(),?,?,'A')",array($Asociado['cedula'],$MontoP*$Asociado['cambio'],$MontoP));               
...
*/                                                                                
    ...
    function Saldo($bd,$cedula){
      $ConSaldo=$bd->dbConsultar("select f.cliente,(select sum(ml.monto_base) from movimientos as ml where ml.cliente=f.cliente and ml.movimiento='Liquidez' and ml.fautoriza between f.inicio and f.fin) Liquidez,(select sum(ml.monto_base) from movimientos as ml where ml.cliente=f.cliente and ml.movimiento='Retiro' and ml.fautoriza between f.inicio and f.fin) Retiros from franquiciados as f inner join movimientos as m on m.cliente=f.cliente where f.cliente=? GROUP BY f.cliente",array($cedula));
      if (!$bd->Error){
    ...
    function SDiferido($bd,$cedula){
      $ConDiferido=$bd->dbConsultar("select sum(m.monto_base) mbase from movimientos as m where fautoriza is null and m.cliente=? and estado='P'",array($cedula));
      if (!$bd->Error){
    ...
    function CDiferido($bd,$cedula){
      $ConDiferido=$bd->dbConsultar("select sum(m.monto_oficial) moficial, sum(m.monto_base) mbase from movimientos as m where fautoriza is null and m.cliente=? and estado='R' and franquicia='REC'",array($cedula));
      //echo $bd->getSql();
      if (!$bd->Error){
    ...
    function RDiferido($bd,$cedula){
        $ConDiferido=$bd->dbConsultar("select sum(m.monto_oficial) moficial, sum(m.monto_base) mbase from movimientos as m where fautoriza is null and m.cliente=? and estado='V' and franquicia='REN'",array($cedula));
        //echo $bd->getSql();
        if (!$bd->Error){
            return $ConDiferido->fetch_array();
            //return $Diferido['mbase'];    
    ...
    function CambioMonetario($bd,$moneda,$fecha){
        $ConCambio=$bd->dbConsultar("select cambio from hmonedas where (? between desde and hasta) or (?>=desde and hasta is null) and id=? ",array(FData($fecha),FData($fecha),$moneda));
        if (!$bd->Error){
