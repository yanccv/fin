...
class dbMysql{
    ...
    #Setteo de Variables
    private function setServidor($server){    $this->Servidor=$server; }
    private function setUser($user){    $this->User=$user; }
    private function setPass($pass){    $this->Pass=$pass; }
    private function setBd($bd){    $this->Bd=$bd; }
    ...
    #Getteo de Variables
    private function getServidor(){    return $this->Servidor; }
    private function getUser(){    return $this->User; }
    private function getPass(){    return $this->Pass; }
    private function getBd(){    return $this->Bd; }
    public function getSql(){    return $this->Sql; }    
    private function  dbEscape($cadena){  return  mysqli_real_escape_string($this->Link,$cadena);    }
        
    protected function getLink(){   return $this->Link;}
            
    public function Moerror(){  if ($this->Error){  $this->MsgError="<center>Error Nro [".$this->Nerror."] ".$this->Merror; return 1;}    }                
    
    public function dbMysql($server=NULL,$user=NULL,$pass=NULL,$bd=NULL){
        if (empty($server)) $this->setServidor($GLOBALS['ServName']); else $this->setServidor($server);
        if (empty($user)) $this->setUser($GLOBALS['UserName']); else $this->setUser($user);
        if (empty($pass)) $this->setPass($GLOBALS['PassWord']); else $this->setPass($pass);
        if (empty($bd)) $this->setBd($GLOBALS['DataBase']); else $this->setBd($bd);        
    } 
    ...
    public function dbDesconectar(){    $this->Link->close();     }
    public function dbRows($consulta){  return $consulta->num_rows;    }
    public function AutoCommit($estado=true){   $this->Link->autocommit($estado);    }
    public function Commit(){   return $this->Link->commit(); }
    public function RollBack(){   $this->Link->rollback(); }
    ...
    public function dbConsultar($sql,$valores=NULL){
        ...
        if (is_object($consulta))   return $consulta;
        else    $this->setError();                        
    }
    public function dbFunction($sql,$valores=NULL){
        ...
        $consulta=$this->dbQuery();   
        //$consulta->errorInfo();    
        if (!$consulta){    $this->setError("Procedimiento No Ejecutado ");    return false;  }                             
        return true;                                
    }
    ...
    public function dbInsertar($sql,$valores){
        ...
        if ($agregar){  return "Registro Agregado Correctamente";      }   
        else    $this->setError();
    ...
    public function dbActualizar($sql,$valores){
        ...
        if ($actualizar){
            ...
            if ($this->Link->affected_rows==1)  return "Registro Actualizado Correctamente";
            if ($this->Link->affected_rows>1)   return "Registros Actualizados Correctamente";
            if ($this->Link->affected_rows==0)  return "Registro No Modificado";            
        }           
    }
    public function dbBorrar($sql,$valores){
        ...
        if ($borrar){
            $this->Affected=$this->Link->affected_rows;
            if ($this->Link->affected_rows==1)  return "Registro Eliminado Correctamente";
            if ($this->Link->affected_rows>1)   return "Registros Eliminados Correctamente";
            if ($this->Link->affected_rows==0)  return "Registro No Eliminado";            
        }   
    }        
    ...
    public function dbComboSimple($sql,$params,$nombre,$codigo,$nombres,$select){
        ...
        if ($this->Error){  echo $this->Moerror(); }
        $comboini="<select name='$nombre' id='$nombre'>\n".
               "<option value='0'>Seleccione</option>\n";
        while ($fila=$consulta->fetch_row()){
            unset($gen);
            for ($i=0;$i<count($nombres);$i++)  $gen.=$fila[$nombres[$i]]." "; 
            if ($select==$fila[$codigo]) $sel="selected='selected'";    else    unset($sel);                       
    ...
    private function dbPrepare($sql, $valores)
    {
        ...
        {
            if(is_bool($valores[$i])){       $valores[$i] = $valores[$i]? 1:0;	}
            elseif(is_double($valores[$i]))  $valores[$i] = str_replace(',', '.', $valores[$i]);
            elseif(is_numeric($valores[$i])) $valores[$i] = $this->dbEscape($valores[$i]);
            elseif(is_null($valores[$i]))	$valores[$i] = "NULL";
            else    $valores[$i] = "'".$this->dbEscape($valores[$i])."'";
        }		
